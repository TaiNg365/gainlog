...
export async function getCardioLogs() {
  const { data, error } = await db
    ...
    .select('*')
    .order('created_at', { ascending: false })
  if (error) { console.error('getCardioLogs:', error); return []; }
  return data.map(r => ({
    id: r.id, date: r.date, machine: r.machine,
    duration: r.duration, calDisplay: r.cal_display,
    notes: r.notes, type: 'cardio'
  }));
}

export async function saveCardioLog(entry) {
  const { error } = await db.from('cardio_logs').upsert({
    id: entry.id, date: entry.date, machine: entry.machine,
    duration: entry.duration, cal_display: entry.calDisplay, notes: entry.notes
  })
  if (error) console.error('saveCardioLog:', error);
}
